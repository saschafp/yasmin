#!/usr/bin/env python3
"""
Benchmark yasmin with the OpenMP backend for the advection-diffusion problem.

Usage: python3 run_yasmin_openmp.py <nx> [nruns]
...
Outputs:
  ...
  COMPILE_TIME_S=<time to compile the OpenMP backend>
...
from yasmin.backends import OpenMPBackend
...
def benchmark_yasmin_openmp(nx: int, nruns: int = 5):
    """Run the same advection-diffusion problem as the GT4Py reference using the OpenMP backend."""
    ny = nx
    cfl = 1.0
    timestep = cfl / (nx - 1) ** 2
    ...
    OpenMPBackend().compile(lower(operator=copy_u_ir, name="copy_u"))
    OpenMPBackend().compile(lower(operator=copy_v_ir, name="copy_v"))
    OpenMPBackend().compile(lower(operator=rk_ir, name="rk_stage"))
    t_compile = time.perf_counter() - compile_t0

    for _ in range(2):
        yasi.execute(copy_u_base, backend="openmp", fields={u_new: u_new_arr.copy(), u_now: np.zeros_like(u_now_arr)}, scalars={})
        yasi.execute(copy_v_base, backend="openmp", fields={v_new: v_new_arr.copy(), v_now: np.zeros_like(v_now_arr)}, scalars={})
        for k in range(3):
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: run_yasmin_openmp.py <nx> [nruns]", file=sys.stderr)
        sys.exit(1)

    nx = int(sys.argv[1])
    nruns = int(sys.argv[2]) if len(sys.argv) > 2 else 5
    benchmark_yasmin_openmp(nx, nruns)
